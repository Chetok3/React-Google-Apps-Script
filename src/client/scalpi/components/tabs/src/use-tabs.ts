...
export function useTabs<T extends object>(originalProps: UseTabsProps<T>) {
  ...
  const { tabListProps } = useTabList<T>(
    otherProps as AriaTabListProps<T>,
    state,
    domRef
  );
  ...
  const getBaseProps: PropGetter = useCallback(
    (s) => ({
      'data-slot': 'base',
      className: slots.base({ class: clsx(baseStyles, s?.className) }),
      ...mergeProps(
  ...
  const placement =
    (variantProps as Props).placement ?? (isVertical ? 'start' : 'top');
  const getWrapperProps: PropGetter = useCallback(
    (s) => ({
      'data-slot': 'tabWrapper',
      className: slots.tabWrapper({
        class: clsx(classNames?.tabWrapper, s?.className),
      }),
  ...
  const getTabListProps: PropGetter = useCallback(
    (s) => ({
      ...
      className: slots.tabList({
        class: clsx(classNames?.tabList, s?.className),
      }),
      ...mergeProps(tabListProps, s),
    }),
